...
function App() {
  ...
  // THIS IS THE ORIGINAL RAIN YOU LOVED — EXACTLY AS IT WAS
  const drops = [];
  ...
  useEffect(() => {
    ...
    for (let x = 0; x < columns; x++) {
      rainDrops[x] = {
        ...
        chars: Array.from({ length: 20 }, () => matrix[Math.floor(Math.random() * matrix.length)])
      };
    }
    ...
    const draw = () => {
      ...
      for (let i = 0; i < rainDrops.length; i++) {
        ...
        const text = drop.chars[Math.floor(Math.random() * drop.chars.length)];
  ...
  // Spawn extra rain on new blocks (your original idea)
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!latest || latest.hash !== block.hash) {
          ...
          // YOUR ORIGINAL TRANSACTION-SPAWNED RAIN BURST
          const canvas = canvasRef.current;
          if (canvas) {
            const ctx = canvas.getContext('2d');
            ctx.fillStyle = '#0ff';
            ...
            for (let i = 0; i < txs.length * 3; i++) {
              setTimeout(() => {
                const x = Math.random() * canvas.width;
                const y = Math.random() * -50;
                const text = "アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワン0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"[Math.floor(Math.random() * 92)];
  ...
  return (
    <>
      ...
      {/* Timeline & Toggle — unchanged */}
      <div style={{
        position: 'fixed', top: '50%', right: isTimelineOpen ? '2vw' : '-360px',
        transform: 'translateY(-50%)', width: '340px', maxHeight: '76vh',
        background: 'rgba(0,10,30,0.96)', borderRadius: '16px', padding: '1.5rem',
        border: '2px solid #0ff', boxShadow: '0 0 40px rgba(0,255,255,0.4)',
        transition: 'right 0.5s cubic-bezier(0.25, 0.8, 0.25, 1)', overflow: 'hidden',
        zIndex: 100, backdropFilter: 'blur(8px)'
      }}>
        <div style={{ height: '100%', overflowY: 'auto', paddingRight: '12px', marginRight: '-12px', scrollbarWidth: 'none' }}>
          <style jsx>{`div::-webkit-scrollbar { display: none; }`}</style>
          {recentBlocks.slice(0, 10).map((b, i) => (
            <div key={b.hash} style={{
              padding: '0.9rem 0', borderBottom: i < 9 ? '1px dashed #033' : 'none',
              color: i === 0 ? '#0f0' : '#0ff', display: 'flex', justifyContent: 'space-between', fontSize: '1.1rem'
            }}>
      ...
      <button
        ...
        style={{
          position: 'fixed', top: '50%', right: isTimelineOpen ? 'calc(2vw + 340px + 12px)' : '12px',
          transform: 'translateY(-50%)', width: '28px', height: '60px',
          background: 'rgba(0, 255, 255, 0.2)', border: '2px solid #0ff', borderRadius: '14px 0 0 14px',
          color: '#0ff', fontSize: '1.4rem', fontWeight: 'bold', cursor: 'pointer',
          boxShadow: '-6px 0 20px rgba(0,255,255,0.6)', transition: 'all 0.4s ease',
          zIndex: 101, outline: 'none', backdropFilter: 'blur(8px)'
        }}
      >
