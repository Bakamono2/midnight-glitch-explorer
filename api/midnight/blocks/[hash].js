// Vercel serverless handler for fetching block transactions via Midnight RPC.
// Exposes GET /api/midnight/blocks/[hash] and returns normalized tx objects.
export default async function handler(req, res) {
  ...
  if (!rpcUrl) {
    res.status(500).json({ error: 'Missing MIDNIGHT_RPC_URL' });
    return;
  }
  ...
  try {
    ...
    console.error('[midnight/blocks/[hash]] error', err);
    res.status(502).json({
